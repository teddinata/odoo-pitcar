    ...
    def get_mechanic_kpi(self, **kw):
        ...
        try:
            ...
            # Handle Head Store KPI
                ...
                # Template data KPI dengan nilai dummy
                dummy_scores = {
                    'revenue_target': {
                        'actual': 92.5,
                        'measurement': "Revenue: Rp 508,750,000 dari target Rp 550,000,000/bulan (92.5%)"
                    },
                    'service_time': {
                        'actual': 87.6,
                        'measurement': "Efisiensi waktu servis: 85.2% (deviasi: 14.8%)\nEfisiensi penerimaan: 90.0% (45/50 dalam 15 menit)\nTotal efisiensi waktu: 87.6%"
                    },
                    'mechanic_efficiency': {
                        'actual': 80.0,
                        'measurement': "Mekanik dalam rentang waktu rata-rata (±5%): 16/20\nRata-rata waktu pengerjaan: 2.5 jam\nRentang target: 2.38 - 2.63 jam"
                    },
                    'customer_satisfaction': {
                        'actual': 100.0,
                        'measurement': "Rating rata-rata: 4.8/5 dari 126 ulasan"
                    },
                    'complaint_handling': {
                        'actual': 95.0,
                        'measurement': "Komplain terselesaikan: 19 dari 20 komplain"
                    },
                    'sop_compliance': {
                        'actual': 92.5,
                        'measurement': "Kepatuhan SOP keseluruhan: 37/40 (92.5%)\n\nDetail per departemen:\n• Service Advisor: 12/13 (92.3%)\n• Mechanic: 15/16 (93.8%)\n• Customer Service: 10/11 (90.9%)"
                    },
                    'parts_availability': {
                        'actual': 93.3,
                        'measurement': "Hari tanpa stockout: 28 dari 30 hari"
                    },
                    'employee_development': {
                        'actual': 85.0,
                        'measurement': "85% karyawan telah mengikuti program pelatihan bulan ini"
                    }
                }
                
                # Add dummy scores for all KPI types in template
                for kpi in head_store_kpi_template:
                    # Get dummy values for this KPI type
                    dummy_data = dummy_scores.get(kpi['type'], {
                        'actual': 90.0,  # Default fallback value
                        'measurement': f"Data dummy untuk {kpi['name']}"
                    })
                    
                    # Calculate weighted score
                    actual = dummy_data['actual']
                    weighted_score = actual * (kpi['weight'] / 100)
                    ...
                    kpi_scores.append({
                        ...
                        'weight': kpi['weight'],
                        'target': kpi['target'],
                        'measurement': dummy_data['measurement'],
                        'actual': actual,
                        'achievement': weighted_score,  # Same as weighted_score
                        'weighted_score': weighted_score,
                        'editable': ['weight', 'target']
                    })
                        ...
                        if kpi['type'] == 'flat_rate':
                            ...
                            if not team_orders:
                                ...
                                if member_averages:
                                    ...
                                    kpi['measurement'] = f"Mekanik dalam rentang target: {mechanics_in_range}/{total_mechanics}, Rata-rata tim: {team_avg:.1f} jam"
                                else:
                                    actual = 0
                                    kpi['measurement'] = f"Tidak cukup data untuk analisis pada periode {month}/{year}"
                            ...
                            if team_orders:
                                ...
                                actual = 0
                                kpi['measurement'] = f"Tidak ada orders pada periode {month}/{year}"
                            ...
                            if total_rated_orders > 0:
                                ...
                                kpi['measurement'] = f"Orders rated: {total_rated_orders}, Puas: {satisfied_customers}, Komplain: {complaints}, Rating: {avg_service_rating:.1f}"
                            else:
                                actual = 0
                                kpi['measurement'] = f"Belum ada rating pada periode {month}/{year}"
                            ...
                            try:
                                ...
                                if not tool_checks:
                                    ...
                                    # Format pesan measurement
                                    kpi['measurement'] = f"Tim hand-tools: {team_matched_items}/{team_total_items} tools sesuai ({actual:.1f}%)"
                    ...
                    for kpi in mechanic_kpi_template:
                        ...
                        if kpi['type'] == 'flat_rate':
                            ...
                            if orders:
                                ...
                                kpi['measurement'] = f"Orders dengan rekomendasi: {orders_with_recs}/{total_orders}, Rata-rata realisasi: {avg_realization:.1f}%"
                                actual = avg_realization
                            else:
                                actual = 0
                                kpi['measurement'] = f"Tidak ada orders pada periode {month}/{year}"
                            ...
                            try:
                                ...
                                if not tool_checks:
                                    ...
                                    # Format pesan measurement
                                    kpi['measurement'] = f"Hand-tools: {matched_items}/{total_items} tools sesuai ({actual:.1f}%)"
