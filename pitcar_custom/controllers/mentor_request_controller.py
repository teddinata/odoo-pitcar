...
class MentorRequestController(http.Controller):
    ...
    def create_request(self, **kw):
        ...
        try:
            ...
            required_fields = ['sale_order_id', 'mechanic_id', 'problem_category', 'problem_description']
            ...
            # Verify that mechanic_id exists
            mechanic = request.env['pitcar.mechanic.new'].sudo().browse(kw['mechanic_id'])
            if not mechanic.exists():
                return {
                    "status": "error",
                    "message": f"Mechanic with ID {kw['mechanic_id']} does not exist"
                }
