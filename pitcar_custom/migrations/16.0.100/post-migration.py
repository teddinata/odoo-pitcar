...
# migrations/16.0.100/pre-migration.py
# migrations/16.0.100/post-migration.py
def migrate(cr, version):
    """
    Migrasi mentor_id dari pitcar.mechanic.new ke hr.employee setelah model dimuat.
    """
    ...
    # Buat environment dengan cursor
    env = api.Environment(cr, 1, {})  # 1 adalah SUPERUSER_ID
    ...
    for request in mentor_requests:
        old_mentor_id = request.mentor_id.id if request.mentor_id else False
        new_mentor_id = mechanic_to_employee_map.get(old_mentor_id) if old_mentor_id else False
    ...
    cr.commit()  # Commit perubahan ke database